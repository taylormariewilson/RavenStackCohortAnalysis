-- Creating v_cohort_onboarding_insights for Tableau integration
CREATE VIEW v_cohort_onboarding_insights AS
...
SupportMetrics AS (
    SELECT 
        b.cohort_month,
        CAST(SUM(CASE WHEN t.submitted_at BETWEEN b.first_signup_date AND DATEADD(day, 30, b.first_signup_date) THEN 1 ELSE 0 END) AS FLOAT) / COUNT(DISTINCT b.account_id) AS avg_tickets_per_user,
        ROUND(AVG(CASE WHEN t.submitted_at BETWEEN b.first_signup_date AND DATEADD(day, 30, b.first_signup_date) THEN CAST(t.first_response_time_minutes AS FLOAT) / 60.0 END), 2) AS avg_response_time_hours
...
SELECT 
    c.cohort_month,
    c.starting_size,
-- CAST (...) AS FLOAT to keep raw decimals (e.g. 0.964) so Tableau can calculate and plot it properly
    CAST(c.month_1_users AS FLOAT) / NULLIF(c.starting_size, 0) AS month_1_retention_rate,
    ROUND(s.avg_tickets_per_user, 2) AS avg_tickets_per_user,
